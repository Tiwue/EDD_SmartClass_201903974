...
class ArbolEstudiantes:
    ...
    def graficarEncriptado(self):
            ...
            if self.raiz is not None:
                cadena= self.graficar_interEncriptado(self.raiz, cadena)
            else:
                cadena += '"Aun no se cargan Estudiantes"'
            cadena += "}"
            s = Source(cadena, filename="Estudiantes",directory='C:\\Users\\steve\\Desktop\\EDD\\EDD_SmartClass_201903974\\EDD_SmartClass_201903974\\Fase3\\static\\files',format='pdf') 
            s.render()


    def graficar_interEncriptado(self, actual, cadena):        
        if actual is not None:
            nombre=actual.estudiante.nombre.decode()
            carrera=actual.estudiante.carrera.decode()
            ...
            if actual.izq != None:
                ...
                cadena = self.graficar_interEncriptado(actual.izq, cadena)
            if actual.der != None:
                cadena += actual.estudiante.carnet + "->"+ actual.der.estudiante.carnet+"\n"
                cadena = self.graficar_interEncriptado(actual.der, cadena )
        return cadena
